...
root_out <- paste0(root_out_base, "_allcells_withsex_noimp_fixedmu")
...
for (ct in ct_keep) {
  ...
  for (cond_lbl in cond_levels_ct) {
    ...
    fitted_vals <- attr(estimates, "fitted_values")
    min_theta <- 1e-6
    theta_vec <- rep(min_theta, nrow(estimates))
    gene_names <- rownames(estimates)
    if (!is.null(fitted_vals)) {
      for (idx in seq_along(fitted_vals)) {
        fv <- fitted_vals[[idx]]
        if (is.null(fv) || !nrow(fv)) next
        cell_idx <- match(fv$cell, colnames(a1))
        valid <- !is.na(cell_idx)
        if (!any(valid)) next
        cell_idx <- cell_idx[valid]
        g <- gene_names[idx]
        k_vec <- as.numeric(a1[g, cell_idx, drop = TRUE])
        n_vec <- as.numeric(tot[g, cell_idx, drop = TRUE])
        mu_vec <- fv$fitted[valid]
        theta_hat <- bb_theta_mle_fixed_mu(k_vec, n_vec, mu_vec)
        if (is.finite(theta_hat) && theta_hat > 0) theta_vec[idx] <- theta_hat
      }
    }
    attr(estimates, "fitted_values") <- NULL
    estimates$bb_theta <- pmax(theta_vec, min_theta)
