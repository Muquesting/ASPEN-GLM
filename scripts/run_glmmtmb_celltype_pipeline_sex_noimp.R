...
## === glm_phi_tests: Use BB LRT with mapped theta (not Wald test) ===
## This function now calls ASPEN's bb_mean() after mapping GLM phi to BB theta
glm_phi_tests <- function(genes, a1, tot, sex_labels, phi_shrunk_vec,
                          min_counts, min_cells, base_mu = 0.5,
                          glob_params = NULL, estimates_with_theta = NULL) {
  
  # This function doesn't actually use phi_shrunk_vec for Wald tests anymore
  # Instead, it uses the mapped theta from estimates_with_theta and calls bb_mean
  
  if (is.null(estimates_with_theta)) {
    warning("glm_phi_tests: estimates_with_theta is NULL, cannot run bb_mean LRT")
    return(NULL)
  }
  
  # Call ASPEN's bb_mean function with the mapped BB parameters
  # bb_mean will do the LRT test using the theta values derived from GLM phi
  bb_mean_results <- tryCatch(
    bb_mean(
      a1_counts = a1,
      tot_counts = tot,
      estimates = estimates_with_theta,
      glob_params = glob_params,
      min_cells = min_cells,
      min_counts = min_counts
    ),
    error = function(e) {
      warning("bb_mean failed: ", e$message)
      NULL
    }
  )
  
  if (is.null(bb_mean_results)) return(NULL)
  
  # Rename columns to match expected output format
  # bb_mean returns: AR, N, log2FC, llr_mean, pval_mean
  # We need: gene, p_intercept, p_sex, pvalue, padj_intercept, padj_sex, padj
  
  result <- data.frame(
    gene = rownames(bb_mean_results),
    statistic = bb_mean_results$llr_mean,
    df = bb_mean_results$N,
    p_intercept = bb_mean_results$pval_mean,  # LRT p-value for intercept (allelic imbalance)
    p_sex = NA_real_,  # bb_mean doesn't test sex effect
    pvalue = bb_mean_results$pval_mean,
    stringsAsFactors = FALSE
  )
  
  result$padj_intercept <- stats::p.adjust(result$p_intercept, method = "BH")
  result$padj_sex <- NA_real_
  result$padj <- result$padj_intercept
...
for (ct in ct_keep) {
  ...
  for (cond_lbl in cond_levels_ct) {
    ...
    message("Fitting GLM (Beta-Binomial Regression) on ", nrow(tot), " genes and ", ncol(tot), " cells (", ct, " / ", cond_lbl, ")…")
    ...
    # 3) Estimate shrinkage hyper-parameters (still useful for reference, but NOT used for tests)
    shrink_vals <- NULL
    ...
    ## BUT glm_phi_tests is modified to ignore phi_shrunk_vec and use phi_hat
    ...
    phi_glm_df <- glm_phi_tests(
      ...
      base_mu        = base_mu,
      glob_params    = glob_params,
      estimates_with_theta = estimates_shrunk
    )
    ...
    phi_glm_df_norm <- glm_phi_tests(
      ...
      base_mu        = base_mu,
      glob_params    = glob_params,
      estimates_with_theta = estimates_norm_shrunk
    )
