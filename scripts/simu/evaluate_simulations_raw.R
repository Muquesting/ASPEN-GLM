...
# Parameters - RAW COUNTS ONLY
sim_dir <- "results/sim_runs/glm_eval_v2"
out_dir <- "results/sim_runs/glm_eval_v2_raw"
...
write.csv(perf_by_category, file.path(out_dir, "plots", "performance_by_category.csv"), row.names = FALSE)
  ...
  for (m in unique(perf$method)) {
    ...
    # We only care about discrimination between Imbalanced (Pos) and Balanced (Neg)
    # So we use all genes
  ...
  p_roc_curves <- ggplot(roc_df, aes(x = 1 - specificity, y = sensitivity, color = method)) +
    ...
    labs(title = "ROC Curves - Simulation Performance",
  ...
  plot_dir <- file.path(out_dir, "plots") # Define plot_dir
  if (!dir.exists(plot_dir)) dir.create(plot_dir, recursive = TRUE) # Ensure directory exists
  ...
  write.csv(auc_df, file.path(plot_dir, "roc_auc_summary.csv"), row.names = FALSE)
