...
for (ct in ct_keep) {
  ...
  message("Fitting GLM-based ASPEN pipeline on ", nrow(tot), " genes and ", ncol(tot), " cells (", ct, ")…")
  ...
  # If only one sex present, skip group tests for this cell type
  if (nlevels(sex_present) < 2) {
    warning("Only one sex present in ", ct, "; skipping group tests.")
  ...
  # Ensure the split.var column in estimates_group is a factor with the same levels
  out_group$estimates_group <- lapply(out_group$estimates_group, function(df) {
    if (!is.null(df) && "sex_group" %in% colnames(df)) {
      # ensure character labels (to match group_* checks that compare to levels(metadata))
  ...
  # Also compute per-condition group estimates (for plotting/validation)
  cond_present <- droplevels(factor(ifelse(is.na(cond), "NA", as.character(cond))))
  if (nlevels(cond_present) >= 2) {
    out_cond <- suppressWarnings(
      estim_glmparams_bygroup(
        a1_counts = a1,
        tot_counts = tot,
        design = design,
        ...
        thetaFilter = 1e-3,
        shrinkAll = FALSE,
        split_var_name = "condition_group"
      )
    )
    # Build metadata with condition grouping and run condition tests
    meta_cond <- meta
    rownames(meta_cond) <- colnames(tot)
    meta_cond$condition_group <- cond_present

    res_cond_mean <- group_mean(
      a1_counts = a1,
      tot_counts = tot,
      metadata = meta_cond,
      split.var = "condition_group",
      min_counts = min_counts,
      min_cells = min_cells,
      estimates = res$estimates_shrunk,
      estimates_group = out_cond$estimates_group,
      equalGroups = FALSE
    )

    res_cond_var <- group_var(
      a1_counts = a1,
      tot_counts = tot,
      metadata = meta_cond,
      split.var = "condition_group",
    ...
    out_cond <- list(estimates_group = NULL)
    res_cond_mean <- NULL
    res_cond_var  <- NULL
  }
  ...
  if (!is.null(out_cond$estimates_group)) {
    saveRDS(out_cond$estimates_group, file = file.path(out_dir, "estimates_by_condition.rds"))
  }
  ...
  # write condition CSVs now that helper is defined
  if (!is.null(out_cond$estimates_group)) {
    to_csv(res_cond_mean, file.path(out_dir, "group_mean_condition_results.csv"),
           cols = c("AR","N","log2FC","llr","pval"))
    to_csv(res_cond_var,  file.path(out_dir, "group_var_condition_results.csv"),
           cols = c("AR","N","log2FC","llr_var","pval_var"))
  }
